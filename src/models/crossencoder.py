...
class CrossEncoderReranker:
    ...
    def predict(
        ...
        # Models like ms-marco-MiniLM output raw logits (range well outside
        # [0, 1]).  Models trained with BCE / sigmoid heads (GTE, BGE, Granite
        # and all our fine-tunes) already return probabilities in [0, 1].
        # Apply sigmoid ONLY when scores fall outside [0, 1].
        if len(scores) > 0 and (np.nanmin(scores) < 0.0 or np.nanmax(scores) > 1.0):
